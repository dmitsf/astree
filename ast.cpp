...
class Suffix {
    ...
    Suffix(int node, int start, int stop):origin_node(node), first_char_index(start), last_char_index(stop) {};
...
class Edge {
    ...
    Edge(int init_first_char_index,
	 int init_last_char_index, int parent_node);
...
void Edge::Remove()
{
	...
	for (;;) {
		...
		return;
	    int r =
		Hash(Edges[i].start_node, T[Edges[i].first_char_index]);
	    if (i >= r && r > j)
		continue;
	    if (r > j && j > i)
		continue;
...
int Edge::SplitEdge(Suffix & s)
{
    ...
    Edge *new_edge = new Edge(first_char_index,
			      first_char_index + s.last_char_index -
			      s.first_char_index,
			      s.origin_node);
    new_edge->Insert();
...
void validate()
{
    ...
    cout << "Leaf count : "
	 << leaf_count << (leaf_count == (N + 1) ? " OK" : " Error!")
	 << "\n";
    cout << "Branch count : "
	 << branch_count
...
int walk_tree(int start_node, int last_char_so_far)
{
	...
	if (edge.start_node != -1) {
		...
		cerr << "Logic error on node " << edge.start_node << '\n';
	    BranchCount[edge.start_node]++;
	    edges++;
	    int l = last_char_so_far;
	    for (int j = edge.first_char_index; j <= edge.last_char_index;
		 j++)
		CurrentString[l++] = T[j];
	    CurrentString[l] = '\0';
